...
"Missing" state in vRA when using Zerto replication to failover to DR.
...
VM Instance UUIDs are changed when using Zerto to failover vRA managed
VMs to DR. This results in VMs changing state to "Missing". To restore
VM Instance UUIDs script uses two CSV files:
...
Refer to the "vRealize Automation Disaster Recovery Using Zerto
Replication" blog post at https://niktips.wordpress.com.
