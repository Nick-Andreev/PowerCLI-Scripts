...
This script should typically be run from the Veeam Backup and Replication server itself. If script needs to be invoked remotely, this parameter specifies the hostname or IP address of the server where Veeam Backup and Replication is running. If script is run from the machine that is not the Veeam Backup and Replication server, VeeamPSSnapIn should also be installed. If the script is invoked locally from the Veeam Backup and Replication server, this is not required.

.EXAMPLE
.\veeam-stats-1.0.ps1

.EXAMPLE
.\veeam-stats-1.0.ps1 -VeeamServer 10.10.10.1
...
foreach ($job in $backup_jobs) {
	...
	# If job's last result is None, job is may still be working. Backup Copy jobs
	# may show they're running even though they are idling, just waiting for the
	# parent job to kick in.
	if($last_result -eq "None" -and $job.IsRunning) {
		if($job.IsRunning -and $job.IsIdle) {
			$last_result = "Idle"
		}
		else {
			$last_result = "Running"
	...
	if(($job.JobType -eq "Backup" -or $job.JobType -eq "Replica" -or $job.JobType -eq "BackupSync") -and
		($job_state -eq "Enabled" -and $last_result -ne "Success" -and $last_result -ne "Running" -and $last_result -ne "Idle"))
	{
		...
		$last_session = Get-VBRBackupSession | where {$_.OrigJobName -eq $job.Name} | Sort creationtime -Descending | select -First 1
		# First determine if there are errors in job's log itself
		foreach($record in $last_session.Logger.GetLog().updatedrecords) {
			if($record.status -eq "EWarning") {
				$warning_messages += "$($job.Name); Job error message is ""$($record.Title)""."
			}
			elseif($record.status -eq "EFailed") {
				$error_messages += "$($job.Name); Job error message is ""$($record.Title)""."
			}
		}
		# Then determine errors for each VM (subtasks)
...
foreach ($job in $surebackup_jobs) {
	# If there're no SureBackup jobs configured, Get-VSBJob returns one empty SureBackup
	# job. Skip it and exit the loop if that's the case.
	if(!$job) {
		continue
	}
...
#region backup repository information
